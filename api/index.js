...
async function run() {
  try {
    ...
    app.get("/bids/user/count", async (req, res) => {
  ...
  try {
    const totalBids = await bidsCollection.countDocuments({ userEmail: email });
    res.json({ totalBids });
  } catch (error) {
    console.error("Failed to count bids:", error);
    res.status(500).json({ error: "Failed to count bids" });
  }
});
